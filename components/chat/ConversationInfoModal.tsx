  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <section className="modal-card conversation-info-v8" role="dialog" aria-modal="true" aria-labelledby="conversation-info-title">
      ...
      {conversation.kind === "dm" ? <div className="conversation-info-dm">
        ...
        <div className="member-panel"><h3>Members</h3><div className="member-list">{members.map((member) => <div className="member-row" key={member.user_id}><button type="button" className="avatar-button-v8" onClick={() => onViewProfile?.(member.profile)}><Avatar name={member.profile.display_name || member.profile.username} path={null} size="small"/></button><span className="grow-copy"><span className="admin-name-line-v7"><strong>{member.profile.display_name}</strong>{member.profile.admin_tag && <span className="admin-badge-v7">{member.profile.admin_tag}</span>}<RoleBadges staffRole={member.profile.staff_role} communityRoles={member.profile.community_roles ?? []} compact hideOwner={Boolean(member.profile.admin_tag)} /></span><small>@{member.profile.username} · {member.role}</small></span>{canManage && member.user_id !== currentUserId && member.role !== "owner" && <button type="button" className="text-button danger-text" onClick={() => void onRemoveMember(member.user_id)}>Remove</button>}</div>)}</div>
