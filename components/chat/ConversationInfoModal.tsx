...
export function ConversationInfoModal({ open, conversation, members, currentUserId, blocked, muted, onClose, onToggleBlock, onSetMute, onReportUser, onUpdateGroup, onAddMember, onRemoveMember, onPin, onArchive, onClear, onDeleteForMe, onOpenSharedMedia, onSearchChat, onViewProfile }: ConversationInfoModalProps) {
  ...
  useEffect(() => {
    if (!open) return;
    setName(conversation.title); setAvatarFile(null); setQuery(""); setResults([]); setMessage(""); setMuteMode(muted ? "forever" : "off");
  ...
  async function saveGroup(event: FormEvent) {
    ...
    try { await onUpdateGroup(name.trim(), avatarFile); setAvatarFile(null); setMessage("Group updated."); }
  ...
  function chooseGroupAvatar(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/") || file.size > 2 * 1024 * 1024) { setMessage("Group pictures must be an image no larger than 2 MB."); return; }
    setAvatarFile(file);
  }
  ...
  async function clearHistory() {
    if (!window.confirm("Clear this conversation history for you? This does not delete messages for other people.")) return;
    await onClear(); onClose();
  }
  async function deleteForMe() {
    if (!window.confirm("Delete this conversation from your list? It can reappear if a new message arrives.")) return;
    await onDeleteForMe(); onClose();
  }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.currentTarget === event.target && onClose()}>
    <section className="modal-card conversation-info-v8" role="dialog" aria-modal="true" aria-labelledby="conversation-info-title">
      ...
      <div className="conversation-quick-grid-v8">
        ...
        <button type="button" onClick={onOpenSharedMedia}><span>▧</span><strong>Media</strong></button>
      ...
      {conversation.kind === "dm" ? <div className="conversation-info-dm">
        <button type="button" className="identity-card identity-card-button-v8" onClick={() => otherProfile && onViewProfile?.(otherProfile)}><Avatar name={conversation.title} path={conversation.avatar_path} size="large"/><div><h3>{conversation.title}</h3><p>{otherProfile?.status_text || (blocked ? "Blocked" : "View profile")}</p></div></button>
        ...
        {canManage && <form className="stack-form" onSubmit={saveGroup}><h3>Group settings</h3><label>Group name<input value={name} onChange={(event) => setName(event.target.value)} maxLength={60}/></label><label className="file-button inline-file-button">Choose group picture<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={chooseGroupAvatar}/></label>{avatarFile && <small>{avatarFile.name}</small>}<button type="submit" className="primary-button fit-button" disabled={saving || !name.trim()}>{saving ? "Saving…" : "Save group"}</button></form>}
        <div className="member-panel"><h3>Members</h3><div className="member-list">{members.map((member) => <div className="member-row" key={member.user_id}><button type="button" className="avatar-button-v8" onClick={() => onViewProfile?.(member.profile)}><Avatar name={member.profile.display_name || member.profile.username} path={member.profile.avatar_path} size="small"/></button><span className="grow-copy"><span className="admin-name-line-v7"><strong>{member.profile.display_name}</strong>{member.profile.admin_tag && <span className="admin-badge-v7">{member.profile.admin_tag}</span>}</span><small>@{member.profile.username} · {member.role}</small></span>{canManage && member.user_id !== currentUserId && member.role !== "owner" && <button type="button" className="text-button danger-text" onClick={() => void onRemoveMember(member.user_id)}>Remove</button>}</div>)}</div>
        {canManage && <div className="add-member-box"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Add someone…"/>{results.length > 0 && <div className="people-results embedded-results">{results.map((profile) => <button key={profile.id} type="button" onClick={async () => { await onAddMember(profile.id); setQuery(""); setResults([]); }}><Avatar name={profile.display_name || profile.username} path={profile.avatar_path} size="small"/><span><span className="admin-name-line-v7"><strong>{profile.display_name}</strong>{profile.admin_tag && <span className="admin-badge-v7">{profile.admin_tag}</span>}</span><small>@{profile.username}</small></span></button>)}</div>}</div>}
