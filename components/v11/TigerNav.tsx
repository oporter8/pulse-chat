...
export function TigerNav() {
  ...
  useEffect(() => {
    ...
    async function syncUser() {
      ...
      const [style, adminResult] = await Promise.all([getV11ProfileStyle(data.user.id, true), supabase.rpc("is_app_admin")]);
      if (cancelled) return;
      setSupporter(Boolean(style.supporter)); setIsAdmin(Boolean(adminResult.data));
    }
    void syncUser();
    const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => {
      if (!session?.user) { setSignedIn(false); setSupporter(false); setIsAdmin(false); return; }
  ...
  return <nav className="tiger-global-nav v12-global-nav v121-nav" aria-label="Tiger Chat navigation">
    ...
    <div className="v121-nav-footer">{supporter && <span className="v121-supporter-mark" title="Tiger Chat supporter">★</span>}<span className="v121-version">v13.2</span></div>
  </nav>;
}
